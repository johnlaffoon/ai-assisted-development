...
```csharp
// Follow .NET naming conventions precisely
...
// ❌ Avoid: Method doing too much
public async Task ProcessOrder(Order order)
{
    // Validates, calculates, saves, sends email, updates inventory...
    // 100+ lines of mixed concerns
}

// ✅ Prefer: Composed, focused methods
public async Task<OrderResult> ProcessOrderAsync(Order order, CancellationToken ct = default)
{
    var validationResult = await ValidateOrderAsync(order, ct);
    if (!validationResult.IsValid)
        return OrderResult.Failed(validationResult.Errors);

    var calculation = CalculateOrderTotals(order);
    var savedOrder = await _orderRepository.SaveAsync(order, ct);
    
    await _eventPublisher.PublishAsync(new OrderProcessedEvent(savedOrder), ct);
...
public decimal CalculateDiscount(Customer customer, Order order)
{
    ...
    if (order.TotalAmount < MinimumDiscountThreshold)
        return 0m;
    
    return order.TotalAmount * customer.DiscountRate;
}
```
...
public sealed class OrderService : IOrderService
{
    ...
    // Focused methods that delegate appropriately...
}
...
// Use primary constructors (C# 12) for simple DI
...
// Always include meaningful messages and inner exceptions
...
```csharp
// Use descriptive test names following: Method_Scenario_ExpectedBehavior
public sealed class OrderServiceTests
{
    ...
    [Fact]
    public async Task SubmitOrder_WithValidDraftOrder_ShouldChangeStatusToSubmitted()
    {
        // Arrange
        var order = Order.Create(CustomerId.New());
        order.AddLine(TestData.CreateProduct(), Quantity.From(1));
    ...
    [Fact]
    public async Task SubmitOrder_WithEmptyOrder_ShouldReturnFailure()
    {
        // Arrange
        var order = Order.Create(CustomerId.New()); // No lines added
    ...
    public async Task SubmitOrder_WithNonDraftOrder_ShouldReturnFailure(OrderStatus status)
    {
        // Arrange
        var order = TestData.CreateOrderWithStatus(status);
...
public sealed class OrdersApiTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    [Fact]
    public async Task CreateOrder_WithValidRequest_ReturnsCreatedWithLocation()
    {
        // Arrange
        var request = new CreateOrderRequest
        {
            CustomerId = Guid.NewGuid(),
            Lines = [new OrderLineRequest { ProductId = Guid.NewGuid(), Quantity = 2 }]
        };
...
public sealed class CustomWebApplicationFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            // Replace database with in-memory provider
...
public sealed class OrderBuilder
{
    ...
    {
        _lines.Add(OrderLine.Create(product, Quantity.From(quantity)));
        return this;
    }
    ...
    public Order Build()
    {
        ...
        // Use reflection or internal methods to set test state
        SetPrivateField(order, "_id", _id);
        SetPrivateField(order, "_status", _status);
...
var order = OrderBuilder.Default()
    ...
    .WithLine(testProduct, quantity: 5)
